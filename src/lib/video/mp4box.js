// This file was built with `npm grunt` inside
// mp4box.js' repository, and a few required changes were made.
// file:src/log.js
/* 
...
ISOFile.prototype.buildTrakSampleLists = function(trak) {	
	...
	for (j = 0; j < stsz.sample_sizes.length; j++) {
		...
		if (j > 0) {
			...
			var trackId = trak.boxes.find(v=>'track_id' in v).track_id
			var initialSample = this.initialSample?.[trackId];
			if (initialSample) {
				initialSample.duration = stts.sample_deltas[stts_run_index];
			}
			sample.dts = initialSample ? (initialSample.dts + initialSample.duration) : 0;
		}
		if (ctts) {
